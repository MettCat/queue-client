...
public class BigQueueUnitTest {
    ...
    public void  queueTest() throws IOException{
        ...
        bigQueue = new BigQueueImpl(testDir, "queueTest100");
        for (int i = 0; i <lists.size(); i++) {
            String list = (String) lists.get(i);
            try {
                bigQueue.enqueue(list.getBytes());
                Thread.sleep(200);
        ...
        for (int i = 0; i <10 ; i++) {
            byte[] b = bigQueue.dequeue();
            String v = new String(b, "utf-8");
            System.out.println("出队---->"+v);
            System.out.println("队列长度---->"+bigQueue.size());
            //System.out.println("队列剩余--->"+new String(bigQueue.peek()));
            if (bigQueue.isEmpty()){
                endpos = 1;
                break;
            }
           /* byte[] c = bigQueue.peek();
            String v1 = new String(c,"utf-8");
            System.out.println("sheng===>"+v1);*/
        }
    }
	...
	public void simpleTest() throws IOException {
		for(int i = 1; i <= 2; i++) {
			...
			bigQueue = new BigQueueImpl(testDir, "simple_test01");
